...
variable "static_routes" {
  ...
  - `routes` - (Optional) Map of routes, where every route is an object with attributes:
  ...
  validation {
    condition     = (length(var.static_routes) > 0 && alltrue(flatten([for static_route in var.static_routes : [for route in static_route.routes : contains(["ip-address", "discard", "next-vr", ""], route.type)]])))
