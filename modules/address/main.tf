locals {
  ...
  dg_addr_obj_normalized   = { for v in local.dg_addr_obj_raw : "${v.name}_${try(v.device_group, "shared")}" => v if var.panorama == true }
  vsys_addr_obj_raw        = flatten([for v in var.vsys : [for i, j in var.addr_obj : merge({ vsys = v }, { name = i }, j)]])
  vsys_addr_obj_normalized = { for v in local.vsys_addr_obj_raw : "${v.name}_${try(v.vsys, "vsys1")}" => v if var.panorama == false }
