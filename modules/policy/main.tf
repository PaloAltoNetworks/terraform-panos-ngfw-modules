...
#    "${try(item.device_group, "shared")}_${replace(try(item.rulebase, "pre-rulebase"), "-","_")}_${replace(try(item.position_keyword, "")," ","_")}_${try(item.position_reference, "")}"
...
resource "panos_panorama_nat_rule_group" "this" {
  for_each = length(var.nat_policy) != 0 ? {
